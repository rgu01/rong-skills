...
{Repeat for five to seven independent new stories, or fewer only when fewer qualify.}
...
- The six `##` section headings and their order are fixed.
- Every `New Stories` block has one unique stable anchor immediately before its
  `###` headline and one `- [ ] Interesting` immediately after that headline.
...
- `New Stories` contains five to seven independent stories unless fewer pass
  the evidence gate. The follow-up section has no numerical limit.
- A selected event appears in exactly one of the two story sections.
