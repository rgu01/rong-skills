...
- Bounded response has two obligations: every pending request is eventually served, and service occurs no later than the inclusive `MAX_WAIT` boundary. A directly observable pending/clock condition needs universal service/liveness evidence as well as universal numeric-bound evidence and non-vacuity. Alternatively, a sound observer or construction may combine both obligations when its checked property detects every unserved or late request.
- Observer adequacy is query-relative. For universal safety such as `A[] not Error`, every late or unserved violation must make `Error` reachable; an optional or nondeterministic error edge can be sound because the universal query explores that violating branch. For eventuality/liveness evidence, all relevant continuations must reach the required service or error outcome as the formula demands. Service and timeout synchronisation/ordering must allow valid service at elapsed time exactly equal to `MAX_WAIT` without an erroneous timeout race, and measurement must stop after service.
